...
class Opts(args: Seq[String]) extends ScallopConf(args) {
  ...
  val cancelledTasks = opt[List[String]]("cancel", short = 'C', descr = "tasks to be cancelled", default = Some(Nil))
  val skippedTasks = opt[List[String]]("skip", short = 'S', descr = "tasks to be skipped", default = Some(Nil))
  ...
  val allForced = opt[Boolean]("force-all", short = 'F', descr = "force all dependent tasks to be rebuilt")

  lazy val cancelledTaskDeps = cancelledTasks().map(Utils.TaskDepParser.apply)
  def isCancelled(td: TaskDep) =
    cancelledTaskDeps.find(_._1 == td.task.name).find(_._2.filterNot(_ == td.args).isEmpty).isDefined
  lazy val skippedTaskDeps = skippedTasks().map(Utils.TaskDepParser.apply)
  def isSkipped(td: TaskDep) =
    skippedTaskDeps.find(_._1 == td.task.name).find(_._2.filterNot(_ == td.args).isEmpty).isDefined
